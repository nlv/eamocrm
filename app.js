															...
															return function (createdAt) {
																return function (closedDate) {
																	return function (statusId) {
																		return {address: address, city: city, closedDate: closedDate, createdAt: createdAt, dateVisit: dateVisit, href: href, id: id, ltype: ltype, master: master, masterSalary: masterSalary, name: name, netWorksCost: netWorksCost, officeIncome: officeIncome, partsCost: partsCost, responsible: responsible, sellCost: sellCost, statusId: statusId, worksCost: worksCost};
																	};
																};
															};
														};
...
var $author$project$App$decodeLeads = $elm$json$Json$Decode$list(
	A2(
		...
		A2(
			...
			A2(
				$elm_community$json_extra$Json$Decode$Extra$andMap,
				A2($elm$json$Json$Decode$field, '_lcreatedAt', $rtfeldman$elm_iso8601_date_strings$Iso8601$decoder),
				A2(
					$elm_community$json_extra$Json$Decode$Extra$andMap,
	...
	function (user, status, master, created_at_from, created_at_to) {
		...
		var master2 = A2(
			...
			A2(
				...
				master));
		var created_at_to2 = A2(
			$elm$core$Maybe$withDefault,
			'',
			A2(
				$elm$core$Maybe$map,
				function (s) {
					return '&created_at_to=' + s;
				},
				created_at_to));
		var created_at_from2 = A2(
			$elm$core$Maybe$withDefault,
			'',
			A2(
				$elm$core$Maybe$map,
		...
		switch (action.$) {
			...
			case 'MasterSelected':
				...
				return _Utils_Tuple2(
					...
					cmd);
			case 'CreatedAtFromSelected':
				var created_at_from = action.a;
				var mfilter = model.filter;
				var created_at_from0 = (created_at_from === '') ? $elm$core$Maybe$Nothing : $elm$core$Maybe$Just(created_at_from);
				var cmd = function () {
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							filter: _Utils_update(
								mfilter,
								{created_at_from: created_at_from0}),
							httpStatus: $author$project$App$Loading('Загружаем заявки...'),
							leads: $elm$core$Maybe$Nothing
						}),
					cmd);
...
var $author$project$App$CreatedAtToSelected = function (a) {
	return {$: 'CreatedAtToSelected', a: a};
};
...
var $author$project$App$viewFilters = function (model) {
	...
	var createdAtToFilter = function () {
		var val = A2($elm$core$Maybe$withDefault, '', model.filter.created_at_to);
		var label = A2(
			$mdgriffith$elm_ui$Element$Input$labelAbove,
			_List_Nil,
			$mdgriffith$elm_ui$Element$text('Создано по'));
		return A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[
			...
			A2(
				...
				{label: label, onChange: $author$project$App$CreatedAtToSelected, placeholder: $elm$core$Maybe$Nothing, text: ''}));
	}();
	var createdAtFromFilter = function () {
		var val = A2($elm$core$Maybe$withDefault, '', model.filter.created_at_from);
		var label = A2(
			$mdgriffith$elm_ui$Element$Input$labelAbove,
			_List_Nil,
			$mdgriffith$elm_ui$Element$text('Создано с'));
		return A2(
			$mdgriffith$elm_ui$Element$el,
			_List_fromArray(
				[
				...
				columns: _List_fromArray(
					[
						...
						header: A2(
							$mdgriffith$elm_ui$Element$el,
							$author$project$App$headerCellStyle,
							$mdgriffith$elm_ui$Element$text('Дата создания')),
						view: function (l) {
							return A2(
								$mdgriffith$elm_ui$Element$el,
								A2(
									...
									$author$project$App$dataCellStyle),
								$mdgriffith$elm_ui$Element$text(
									A4($CoderDennis$elm_time_format$Time$Format$format, $CoderDennis$elm_time_format$Time$Format$Config$Config_ru_ru$config, '%d.%m.%Y %I:%M:%S', $elm$time$Time$utc, l.createdAt)));
						},
						width: $mdgriffith$elm_ui$Element$fill
					},
						{
